...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen w-full bg-background text-foreground">
      ...
      {/* Main Content Area with Mobile Header & Mobile Sheet */}
      <div className="flex flex-1 flex-col min-w-0">
        {/* Mobile Header (Visible only on mobile / small screens < lg) */}
        <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-border bg-background/95 px-4 backdrop-blur lg:hidden">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold tracking-tight">Engineering OS</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex h-7 w-7 items-center justify-center rounded-md bg-gradient-to-br from-primary to-chart-3 text-[11px] font-semibold text-primary-foreground">
        ...
        <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
          <SheetContent side="left" className="w-72 p-0 bg-sidebar border-r border-border">
            <SheetHeader className="sr-only">
              ...
              <SheetDescription>Main navigation sidebar for Engineering OS</SheetDescription>
            </SheetHeader>
            <SidebarNavContent
              pathname={pathname}
              userProfile={userProfile}
...
export function PageHeader({
  ...
  return (
    <div className="flex flex-wrap items-end justify-between gap-4 border-b border-border/60 px-4 py-5 sm:px-8 sm:pt-8 sm:pb-6">
...
export function PageBody({ children }: { children: ReactNode }) {
  return <div className="px-4 py-6 sm:px-8 sm:py-8">{children}</div>;
}