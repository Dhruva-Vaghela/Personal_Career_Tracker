...
import { Shield, Plus, Target } from "lucide-react";
...
export const Route = createFileRoute("/security")({
  head: () => ({
    meta: [
      { title: "Cyber Security · Engineering OS" },
      { name: "description", content: "Building secure systems." },
    ],
  }),
...
function SecurityPage() {
  const topics = useAppStore(state => state.securityTopics);
  const addTopic = useAppStore(state => state.addSecurityTopic);
      ...
      <PageHeader 
        ...
        actions={
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Add Security Topic</DialogTitle>
              </DialogHeader>
              <form onSubmit={handleAdd} className="space-y-4 pt-4">
                <div className="space-y-2">
                  <Label>Topic Title</Label>
                  <Input value={newTopic.title} onChange={e => setNewTopic({...newTopic, title: e.target.value})} placeholder="e.g. OWASP Top 10" required />
                </div>
                <div className="space-y-2">
                  <Label>Status</Label>
                  <select 
      ...
      <PageBody>
        {topics.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center text-muted-foreground border border-dashed border-border/50 rounded-lg bg-background/20">
            ...
            <p className="text-[13px]">No security topics mapped yet.</p>
          </div>
        ) : (
