...
export const Route = createFileRoute("/ai")({
  head: () => ({
    meta: [
      { title: "AI Engineering · Engineering OS" },
      { name: "description", content: "LLMs, RAG, agents, evals." },
    ],
  }),
  component: AIPage,
});
...
function AIPage() {
  const topics = useAppStore(state => state.aiTopics);
  const addTopic = useAppStore(state => state.addAiTopic);
  ...
  return (
    <>
      <PageHeader 
        ...
        description="Ship models like software." 
        actions={
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Add AI Topic</DialogTitle>
              </DialogHeader>
              <form onSubmit={handleAdd} className="space-y-4 pt-4">
                <div className="space-y-2">
                  <Label>Topic Title</Label>
                  <Input value={newTopic.title} onChange={e => setNewTopic({...newTopic, title: e.target.value})} placeholder="e.g. LangGraph Agents" required />
                </div>
                <div className="space-y-2">
                  <Label>Status</Label>
                  <select 
      ...
      <PageBody>
        {topics.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center text-muted-foreground border border-dashed border-border/50 rounded-lg bg-background/20">
            ...
            <p className="text-[13px]">No AI topics mapped yet.</p>
          </div>
        ) : (
